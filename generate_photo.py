...
def genererate_pictures(_path, num_samples,w,h,min_fig_size):
    ...
    for i in range(num_samples):
        ...
        img_box = draw_boxes_random_rotated(w=w, h=h, fc=random_color(), mfs=min_fig_size, num_shapes=3)
        img_box.save(os.path.join(os.path.join(_path, "boxes"), f"box-{i}.png"))

        # Генерация круга с одним из 7 цветов
        img_circle = draw_circles_random_rotated(w=w, h=h, fc=random_color(), mfs=min_fig_size, num_shapes=3)
        img_circle.save(os.path.join(os.path.join(_path, "circles"), f"circle-{i}.png"))

        # Генерация круга с одним из 7 цветов
        img_treangle = draw_triangles_random_rotated(w=w, h=h, fc=random_color(), mfs=min_fig_size,num_shapes=3)
